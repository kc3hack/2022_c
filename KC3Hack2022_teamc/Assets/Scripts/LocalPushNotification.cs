...
public static class LocalPushNotification
{
    ...
    // プッシュ通知を登録します。    
    public static void AddSchedule(string title, string message, int badgeCount, DateTime fireTime, string cannelId)
    {
#if UNITY_ANDROID
        SetAndroidNotification(title, message, badgeCount, fireTime, cannelId);
#endif
#if UNITY_IOS
    ...
    // 通知を登録(Android)   
    static private void SetAndroidNotification(string title, string message, int badgeCount, DateTime fireTime, string cannelId)
    {
        // 通知を作成します。
